    ...
    for p in data['annotations']:
      ...
      for p in data['annotations']:
        if (p['image_id'] == counter) and p['category_id']-1<3:
          print(p['category_id']-1, end=" ", file=open(destination_path +'/FLIR_0{:04d}.txt'.format(counter + 1), 'a'))
...
source_path="./flir/FLIR_JSON/thermal_annotations_test.json" #train valid o test
destination_path="./flir/test" #train valid o test
...
for i in range(1,8862):
  my_file = "./flir/training/FLIR_0{:04d}.txt".format(i)
  #print(my_file)
  #print(i)
