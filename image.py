...
def load_data_detection(imgpath, shape, crop, jitter, hue, saturation, exposure):
    #print("path:", imgpath)
    #imgpath="../../../../data/datasets/KAIST_MPD/rgbt-ped-detection/data/kaist-rgbt/images/"
    #if dataset=="FLIR":
     #   labpath = imgpath.replace('images', 'annotations').replace('/lwir/', '/').replace('.jpg', '.json').replace('.jpeg', '.txt').replace('.png','.txt').replace('.tif','.txt').replace('RGB', 'Annotations')
    #else: #ALLORA KAIST
     #   labpath = imgpath.replace('../../../../data/datasets/KAIST_MPD/rgbt-ped-detection/data/kaist-rgbt/images', './kaist/annotations').replace('.jpg', '.txt')
    #labpath = imgpath.replace('../../../../data/datasets/KAIST_MPD/rgbt-ped-detection/data/kaist-rgbt/images',
     #                         './kaist/annotations').replace(
    ...
    #labpath = imgpath.replace('../../../../data/datasets/FLIR_ADAS/FLIR_ADAS/training/RGB','./flir/training')
    #ANCHE EVAL_ALL DA CAMBIARE
    ## data augmentation
    #print("check annotation: ",labpath)
    #labpath="./annotations1/set06/V000/prova.txt"
    #imgpath="../../../../data/datasets/FLIR_ADAS/FLIR_ADAS/training/RGB/FLIR_02888.jpg"
