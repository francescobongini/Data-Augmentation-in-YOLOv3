...
kaist_img_path ="../../../../data/datasets/KAIST_MPD/rgbt-ped-detection/data/kaist-rgbt/images/"
...
kaist_label_path = '../../../../data/datasets/KAIST_MPD/rgbt-ped-detection/data/kaist-rgbt/annotations/'


#transformed lables to save path
kaist_label_tosave_path = './kaist/annotations/'
kaist_list_tosave_path = './kaist/'
...
kaist_names = open('./data/kaist_person.names','r') #only person
...
for phase in ['train','test']: #train_set, test_set
    ...
    for set_ in allset: #set0x
        ...
        for V00_ in all_V:
            ...
            '''
            if set_ == 'set06' and set_counter == 9901: #all = 1161, person only = 901
...
'''
            ...
            for indexi, img in enumerate(kaist_images): #img is the name file, indexi is the iter
                #all train images but 20% from test
                '''
                if set_ == 'set06' and set_counter == 1500:
                    print('{0} has reached {1}'.format(set_,set_counter))
                    time.sleep(1)
                    break
                elif set_ == 'set07' and set_counter == 514:
...
'''
                ...
                kaist_label_totest_path = kaist_label_path + set_ + '/' + V00_  + '/' + filename.replace('.jpg', '.txt')
                ...
                if len(label_contents)<2:
                    kaist_label_tosave = save_path + filename.replace('.jpg', '.txt')
                    open(kaist_label_tosave, 'w')
                ...
                for line in label_contents:
                    ...
                    if line != '': #check if there must be an object
                        ...
                        if(len(data) == 12):
                            ...
                            if class_str == 'person?' or class_str == 'person':# or class_str == 'people': #considerare people come person o no?
                                class_str = 'person'
                            else: #uncomment this for excluding people and cyclist
                                continue #go to next line
                            #create a txt file for annotations
...
for i in range(0,1539):
  my_file = "./kaist/annotations/set11/V001/lwir/I0{:04d}.txt".format(i)
  #print(my_file)
  #print(i)
